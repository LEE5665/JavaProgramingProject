...
public class TodoPanel extends JPanel {
    ...
    private void reloadTodosByTitle() {
        ...
        List<Todo> todos = TodoDAO.listAll(userId).stream()
    ...
    private void openTodoEditor(Todo todo) {
        TodoEditorFrame.TriConsumer<String, LocalDate, LocalDate> onSave = (title, start, end) -> {
            if (todo == null) {
                Todo newTodo = new Todo(userId, null, 0, title, start, end);
        ...
        TodoEditorFrame frame = new TodoEditorFrame(
                ...
                todo == null ? LocalDate.now() : todo.getStartDate(),
                todo == null ? LocalDate.now() : todo.getEndDate(),
                onSave
        );
